...
void calibrateCameraFromVideo(const std::filesystem::path &videoPath, const std::filesystem::path &dataPath)
{
    ...
    runCalibration(rQOi_set,imageSize,param);
...
// ------------------------------------------------------------
void runCalibration(const std::vector<std::vector<cv::Point2f>> & rQOi_set,  const cv::Size & imageSize, CameraParameters & param){

    std::vector<cv::Point3f> rPNn_base;
    std::vector<std::vector<cv::Point3f>> rPNn;
    generateCalibrationGrid(rPNn_base);
...
// ------------------------------------------------------------
void generateCalibrationGrid(std::vector<cv::Point3f> & rPNn_grid){
    ...
    for(int i=0; i < gridHeight; i++) {
        for(int j=0; j < gridWidth; j++){
            rPNn_grid.push_back(cv::Point3f(j*squareSize,i*squareSize,0));
        }
    }
}
